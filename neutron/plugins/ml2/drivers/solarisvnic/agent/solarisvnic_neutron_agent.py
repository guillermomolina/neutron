...
from neutron.conf.plugins.ml2.drivers import solarisvnic as config
from neutron.api.rpc.handlers import securitygroups_rpc as sg_rpc
from neutron.plugins.ml2.drivers.agent import _agent_manager_base as amb
from neutron.plugins.ml2.drivers.agent import _common_agent as ca

from neutron.plugins.ml2.drivers.solarisvnic.common import config  # noqa
from neutron.plugins.ml2.drivers.solarisvnic.common \
    import constants as s_constants
...
class SolariVNICsNetworkManager(amb.CommonAgentManagerBase):
        ...
        if self.interface_mappings:
            ...
            for device in devices:
                ...
                LOG.error("Unable to obtain MAC address for unique ID. "
                          "Agent terminated!")
                sys.exit(1)
        agent_id = 'solarisvnic%s' % mac.replace(":", "")
    ...
    def get_extension_driver_type(self):
        LOG.debug("get_extension_driver_type %s ", s_constants.EXTENSION_DRIVER_TYPE)
        return s_constants.EXTENSION_DRIVER_TYPE
...
def parse_interface_mappings():
    if not cfg.CONF.SOLARISVNIC.physical_interface_mappings:
    ...
    try:
        interface_mappings = helpers.parse_mappings(
            cfg.CONF.SOLARISVNIC.physical_interface_mappings)
...
def validate_firewall_driver():
    ...
    if fw_driver not in supported_fw_drivers:
        LOG.error('Unsupported configuration option for "SECURITYGROUP.'
                  ...
                  'supported by solarisvnic agent, but "%s" is configured. '
...
def main():
    ...
    manager = SolariVNICsNetworkManager(interface_mappings)
    ...
    agent = ca.CommonAgentLoop(manager, polling_interval,
                               ...
                               s_constants.SOLARISVNIC_AGENT_BINARY)
